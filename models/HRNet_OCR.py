...
if torch.__version__.startswith('0'):
    ...
    # BatchNorm2d_class = BatchNorm2d = torch.nn.SyncBatchNorm # 放弃分布式训练
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.size = (config.TRAIN.IMAGE_SIZE[0], config.TRAIN.IMAGE_SIZE[1])

        # stem net
        self.conv1 = nn.Conv2d(config.DATASET.NUM_CHANNELS, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
    ...
    def forward(self, x):
        ...
        # ocr
        out_aux = self.aux_head(feats)# 特征图经过两个1x1卷积层
        # compute contrast feature
        feats = self.conv3x3_ocr(feats)# 特征图经过一个3x3卷积层
        ...
        out_aux = F.interpolate(out_aux, size=self.size, mode='bilinear', align_corners=ALIGN_CORNERS)
        out = F.interpolate(out, size=self.size, mode='bilinear', align_corners=ALIGN_CORNERS)
